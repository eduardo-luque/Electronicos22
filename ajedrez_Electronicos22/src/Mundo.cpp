...
void Mundo::Dibuja(int num, char tipo) {   //llamamos al tablero para dibujarlo. 
	if (tipo == 'a') {
		Dibuja1(num, tipo);
	}
	if (tipo == 'b') {
...
void Mundo::Dibuja1(int num, char tipo) {
	tabl.Dibuja(mast, num, tipo);
	DibujaCasilla(origen_n);
	DibujaCasilla(origen_b);
}

void Mundo::Dibuja2(int num, char tipo) {   //llamamos al tablero para dibujarlo. 
	tabl.Dibuja(mast, num, tipo);
	DibujaCasilla(origen_n);
	DibujaCasilla(origen_b);
}

void Mundo::Raton(int x, int y, int b, bool state, char t) { //CON ESTA FUNCION UTILIZAREMOS EL MOUSE
...
int Mundo::MovimientoPlayer(char t) {
	...
	//para evitar llamar tres veces a la función solo la llamamos una única vez y evaluamos su resultado:
		...
		else if (resultadoNegras == 3) { return 2; }
	...
	//para evitar llamar tres veces a la función solo la llamamos una única vez y evaluamos su resultado:
		...
		if (resultadoBlancas == 2) { //movimiento simple sin comer
			...
			if (mo.jaque_negro({ origen_b, destino_b }, mast)) { //si hay jaque
				return 3; //jaque negras
			}
			if (mo.jaque_blanco({ origen_b, destino_b }, mast) == true) {
