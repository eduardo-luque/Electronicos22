...
Coordinador::Coordinador() {  // empezamos en la pantalla de inicio
	// aqui definimos el estado inicial correspondiente al menú y el tema, que seguirá sonando hasta que cambiemos de estado
	estado = INICIO;
...
void Coordinador::Dibuja() {
	if (estado == INICIO) {   //dibujamos la pantalla de inicio
		...
		ETSIDI::printxy("PULSE E PARA ELEGIR TIPO", -10, 5);
		...
		ETSIDI::setTextColor(1, 1, 1);
		ETSIDI::setFont("fuentes/Bitwise.ttf", 30);
		ETSIDI::printxy("PULSE A PARA LOS TEMATICOS", -12, 7);
		ETSIDI::printxy("PULSE B PARA LOS CLASICO", -12, 5);
		ETSIDI::printxy("PULSE S PARA SALIR", -12, 3);
		ETSIDI::setFont("fuentes/Bitwise.ttf", 15);
	}
	else if (estado == INICIO1) {
		glDisable(GL_LIGHTING);
		glColor3ub(0, 0, 0);
		glutSolidSphere(10, 10, 10);
		glEnable(GL_LIGHTING);
...
void Coordinador::Tecla(unsigned char key) {
	if (estado == INICIO) {
		...
		if (key == 'E' || key == 'e') { estado = SELECCION; }
		else if (key == 'S' || key == 's') { exit(0); }  
	}
	else if (estado == SELECCION) {
		if (key == 'A' || key == 'a') {
			set_tipo('a'); estado = INICIO1;
		}
		if (key == 'B' || key == 'b') {
			set_tipo('b'); estado = INICIO1;
		}
...
void Coordinador::Raton(int b, int state, int x, int y) {
	if (estado == INICIO1) {
		...
		if (b == 0 && state == 1 && x >= 126 && x <= 470 && y >= 340 && y <= 367) {
			...
			turno = JUGADOR_B;
			estado = MULTIPLAYER;
			cout << "raton pulsado en multiplayer" << endl;
		}
		if (b == 0 && state == 1 && x >= 126 && x <= 470 && y >= 397 && y <= 421) {
			cout << "raton pulsado en salir" << endl;
			exit(0);
		}
	}
