...
bool Movimientos::jaque_negro(move_t ma,Master& ta) 
{
	...
	int ocupada = 0;
	for (int i = 0; i < 9; i++) {   //detectar posicion rey negro en todo momento
		for (int j = 0; j < 9; j++) {
	...
	for (int r = 0; r < 9; r++) {   //detectar posicion rey negro en todo momento
		for (int t = 0; t < 9; t++) {
				cout << "Rey negro esta en la casilla " << reyn_x << ", " << reyn_y << endl;
				switch (ta.peones[r][t]->type()) {
				case(Pieza::PEON_BLANCO):
					pb = peon.movimiento(Pieza::PEON_BLANCO, ma);
					if (pb)
					{
						//cout << "Hay jaque negras de peon? " << endl;
						if (ma.dest.x == reyn_x && ma.dest.y == reyn_y)
						{
							cout << "Hay jaque negras " << endl;
							return true;
						}
					}
					break;
				...
				case(Pieza::CABALLO_BLANCO):
					...
					break;
				case(Pieza::REINA_BLANCA): //pruebo jaque con la reina blanca
					reinab = reina.movimiento(Pieza::REINA_BLANCA, ma);
					if (reinab)
					{
						for (int k = 0; k < 9; k++) {
							//cout << "Hay jaque negras de reina? " << endl;
							if (ma.dest.x+k == reyn_x && ma.dest.y+k == reyn_y)
							{
								cout << "Hay jaque negras " << endl;
								return true;
							}
